...
def get_users_top(auth_header, t, tr, lim):
    terms = ['short_term', 'medium_term', 'long_term']
    if t not in ['artists', 'tracks']:
        print('invalid type')
        return None
    if tr not in terms:
        print('invalid term specified- options are: {}'.format(terms))
        return None
    if int(lim) < 0 or int(lim) > 50:
        print('invalid limit- options are: 0-50')
        return None
